...
test('Currying a function with multiple arguments', assert => {
  let testFunction = (a, b, c, d) => a + b + c + d;
  let testCurry = funcker.curry(testFunction);
...
test('Currying a function with multiple arguments', assert => {
  let testFunction = (a, b, c, d) => a + b + c + d;
  let testCurry = funcker.curry(testFunction, 't', 'e', 's');
